...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> onTapToNextButton() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (Platform.isIOS) {
        // Ensure APNs token exists before getting FCM token
        String? apnsToken = await FirebaseMessaging.instance.getAPNSToken();
        if (apnsToken == null) {
          debugPrint("⏳ APNs token not available yet. Waiting...");
          // Option A: wait for refresh
          FirebaseMessaging.instance.onTokenRefresh.first.then((newToken) {
            debugPrint("📱 iOS FCM Token received via refresh: $newToken");
          });
        } else {
      ...
      // 🔑 Sign in with email, password, and FCM token
